...
vWinWidth := MonitorWorkAreaRight - MonitorWorkAreaLeft
hWinWidth := vWinWidth // 2
vWinHeight := MonitorWorkAreaBottom - MonitorWorkAreaTop
hWinHeight := vWinHeight // 2
...
onKeyDown(keyName){
	global gDownKeyName,fstate,flag,gmode,SMODE,RMODE,TMODE,KMODE,nowkey
	nowkey := A_ThisHotkey
	if(fstate != ""){
		flag := 1
...
#IfWinActive ahk_class gsview_class ;gsviewでAlt+eでps to epsを選択
!e::PostMessage, 0x111, 105, 0, , ahk_class gsview_class
#IfWinActive
^Esc::Suspend,Toggle
...
processE(){
	global hWinHeight,vWinWidth,fstate,MonitorWorkAreaTop,MonitorWorkAreaLeft
	if(fstate=="M"){
		WinMinimize,A     ; 最小化
	}else if(fstate=="MH")
		WinMove,A,, %MonitorWorkAreaLeft%, %MonitorWorkAreaTop%, %vWinWidth%, %hWinHeight%
	else if(fstate=="HM")
		WinMove,A,, % MonitorWorkAreaLeft, (hWinHeight+MonitorWorkAreaTop), vWinWidth, hWinHeight
	else if(fstate=="MS")
		Send,#e
	return
}
processR(){
	global hWinWidth,vWinHeight,fstate,MonitorWorkAreaTop,MonitorWorkAreaLeft
	if(fstate=="M"){
		...
		if(A_OSVersion=="WIN_7" || A_OSVersion=="WIN_8" || A_OSVersion=="WIN_8.1")
			...
			WinMove,A,, % (hWinWidth+MonitorWorkAreaLeft), MonitorWorkAreaTop, hWinWidth, vWinHeight
	}else if(fstate=="HM"){
		if(A_OSVersion=="WIN_7" || A_OSVersion=="WIN_8" || A_OSVersion=="WIN_8.1")
			Send,#{LEFT}
		else
			WinMove,A,, %MonitorWorkAreaLeft%, %MonitorWorkAreaTop%, %hWinWidth%, %vWinHeight%
	}else if(fstate=="MS")
		Send,#r
	return
}
...
processG(){
	global fstate,groupNum,groupIDs
	if(fstate=="M"){
		
		GroupActivate, Group%groupNum%
...
;		GroupAdd, GroupName,
		...
		IfInString, groupIDs, %active_id%
		{
			StringReplace, groupIDs, groupIDs, |%active_id%,, ALL
			groupNum++
			StringSplit, arrGroupIDs, groupIDs, |
			tmpstr := ""
			Loop, %arrGroupIDs0%
			{
				if (A_Index == 1)
					Continue
				tmpid := arrGroupIDs%A_Index%
				IfWinExist, ahk_id %tmpid%
				{
					tmpstr := tmpstr . "|" . tmpid
					GroupAdd, Group%groupNum%, ahk_id %tmpid%
				}
			}
			groupIDs := tmpstr
		}
		...
		{
			GroupAdd, Group%groupNum%, ahk_id %active_id%
			groupIDs := groupIDs . "|" . active_id
		}
	}
	return
}
	...
	if(fstate=="M"){
		...
		Loop,%macronum%{
			...
			Loop{
				...
				if(matchPos = 0){
					processMacroSend(sndmcr)
					;Send,%sndmcr%
					break
				}else if(matchPos <> 1){
					StringLeft, temp,sndmcr,% (matchPos-1)
					processMacroSend(temp)
				}
				;Escで中断
		...
		MouseGetPos, xpos, ypos
		InputBox,ipt,Macro%chr% (Esc makes the macro cancel),{UP}{LEFT}{RIGHT}{DOWN}{HOME}{END}{PgUp}{PgDn}`n{TAB}{BS}{DEL}{ENTER}{INS}{Esc}{AppsKey}`n^:Ctrl !:Alt +:Shift #:Window ``increment`` {hk MHr}:key MH+r`n{Click`,%xpos%`,%ypos%`,Right`,Down} : Right mouse down at Mpointer`n{Wait 1000} : Wait 1000ms  {Calc (1+2)*3} : 9 [use+-*/^`%()],,,,,,,,% macro%chr%
		if ErrorLevel = 0
			macro%chr% := ipt
	}else if(fstate=="HM"){
...
processMacroSend(tempstr){
	global fstate,nowkey
	;Send,%tempstr%
	;Hotkeyを使えるように
	Loop{
		matchPos := RegExMatch(tempstr,"i)`{hk ([MSH]+)([a-z@;\:,.\/])`}",matchStr)
		if(matchPos = 0){
			Send,%tempstr%
			break
		}else if(matchPos <> 1){
			StringLeft, temp,tempstr,% (matchPos-1)
			Send,%temp%
		}
		;Escで中断
		GetKeyState, state, Esc, P
		...
		fstate := matchStr1
		nowkey := matchStr2
		StringUpper, matchStr2, matchStr2
		if (matchStr2 == "@")
			matchStr2 := "AT"
		else if (matchStr2 == ";")
			matchStr2 := "SColon"
		...
		StringLen,matchLen,matchStr
		cutnum := matchPos+matchLen-1
		StringTrimLeft,tempstr,tempstr,%cutnum%
	}
}
kaiseki(siki){
	Loop{
